...
def parse_args():
    parser = argparse.ArgumentParser(description='Preprocess notes and prepare for running MixEHR. Separating tokens of different data types but mark them as one type.')
            ...
            for _, row in data[data['HADM_ID'] == id].iterrows(): # for one HADM_ID - CATEGORY pair
                ...
                for word in set(row["PROCTEXT"].split()):
                    if word not in word_indexes[row['CATEGORY']].keys():
                        continue
                    if not args.no_data:
                        output.append(" ".join([str(int(id)), "1", str(word_indexes[row['CATEGORY']][word]), "0", str(unigram_counts[word])]))
                    if not set_label:
                        set_label = True
...
if __name__ == '__main__':
    ...
    merged_data = data[data['CATEGORY'].isin(categories)][["HADM_ID", "CATEGORY", "MORTALITY_LABEL"]].drop_duplicates()
    if args.discard_ids:
    ...
    if not args.use_vocab:
        ...
        offset = 0
        for word_index in word_indexes.values():
            for word, word_id in word_index.items():
                word_index[word] = word_id + offset
            offset = np.max(list(word_index.values())) + 1
    ...
    output, mortality_label = create_output_dataframe(merged_data, word_indexes, args)
    ...
    if args.meta_data:
        meta_data = []
        for category in categories:
            meta_data += [" ".join(["1", str(idx), "1"]) for idx in word_indexes[category].values()]
