...
def get_file_name(num_pats, num_tokens, vocab, iteration, t=None):
    ...
    if t:
        sim_file_name = folder_name+"sim_phi_type"+str(t)+"_"+file_name_prefix+".pkl"
        mxr_file_name = folder_name+"sim_mxr_data_type"+str(t)+"_"+file_name_prefix+"_JCVB0_nmar_K50_iter"+str(iteration)+"_phi_normalized.csv"
    else:
        sim_file_name = folder_name+"sim_phi_"+file_name_prefix+".pkl"
        mxr_file_name = folder_name+"sim_mxr_data_"+file_name_prefix+"_JCVB0_nmar_K50_iter"+str(iteration)+"_phi_normalized.csv"
...
def plot_for_types():
    ...
    iters = {1:361,  2:420, 4:519} # update this dictionary for the number of iterations the model was trained for each note-type
    type_results = {}
    for t in iters.keys():
...
def pearson_corr(sim_topics_file, mxr_topics_file, types=None):
    ...
    for type_id in types:
        chosen_words = mxr_topics[mxr_topics[0] == type_id][mxr_topics.columns[1]]
        sim_topics_df = pd.DataFrame(sim_topics[type_id])
#        print(chosen_words-1)
        sim_topics_df =  sim_topics_df[chosen_words-1]
        mxr_topics_df = mxr_topics[mxr_topics[0] == type_id][mxr_topics.columns[2:]]


        p_corr = find_corr(sim_topics_df.values, mxr_topics_df.values)
        p_corr_max = np.max(p_corr, axis=0)

        pearson_correlations[type_id] = p_corr_max

    return pearson_correlations
    
def main():
    pkl.dump(plot_for_vocab(), open("results/vocab_corr_new.pkl","wb"))
    pkl.dump(plot_for_types(), open("results/types_corr_new_300.pkl","wb"))
    pkl.dump(plot_for_drop(), open("results/drop_corr_new.pkl","wb"))
    pkl.dump(plot_for_patients(), open("results/patients_corr_new.pkl","wb"))
    pkl.dump(plot_for_tokens(), open("results/tokens_corr_new.pkl","wb"))
